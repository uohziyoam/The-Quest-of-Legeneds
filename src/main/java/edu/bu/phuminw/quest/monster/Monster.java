...
public abstract class Monster implements Creature, Damagable {
    private String name;
    private Mark mark;
    private double hp;
    private int level;
    ...
    public boolean attack(Damage dmg) {
        if (!canDodge()) {
            ...
            hp = (finalDamage >= hp) ? 0 : hp - finalDamage;
            System.out.printf("[!] %s was attacked %.2f%s\n", name, finalDamage, (hp == 0) ? " and died" : "");
    ...
    public String toString() {
        return String.format("Lv %d damage %.2f defense %.2f dodge %.2f", getName(), getLevel(), getDamage().getDamage().getFirst(), getDefense(), getDodge());
    }
    ...
    public boolean equals(Object other) {
        ...
        return (this.name == m.getName()) && (this.hp == m.getHp()) && (this.level == m.getLevel()) && (this.defense == m.getDefense()) && (this.dodge == m.getDodge());
    }
}